...
function lovelaceForAddress(rows, address) {
  ...
  return rows.reduce((sum, row) => (row?.address === address ? sum + lovelaceFromAmounts(row.amount) : sum), 0);
}
...
function normalizeCardanoTransaction(transaction, address) {
  ...
  const counterparty = Array.isArray(counterpartyRows)
    ? counterpartyRows.find((row) => row?.address && row.address !== address)?.address || null
    : null;
