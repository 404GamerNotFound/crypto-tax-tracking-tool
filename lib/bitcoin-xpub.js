...
function inspectXpub(value) {
  const extendedKey = normalizeExtendedPublicKey(value);
  ...
  try {
    ...
    if (/non-base58 character/i.test(String(error.message))) {
      throw new Error("Der Extended Public Key enthält ein ungültiges Zeichen. Bitte nur den vollständigen xPub, yPub oder zPub ohne Zusatztext einfügen.");
    }
